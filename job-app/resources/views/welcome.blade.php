<x-main-layout>
    <div x-data="{ show: false }"  x-init="setTimeout(() => show = true, 300)">
        <div class="inline-flex items center mb-2" x-cloak x-show="show" x-transition:enter="transition ease-out duration-700"
         ...
         <h1 class="text-sm  text-white/60 rounded-full bg-white/10 px-2 py-1 w-fit">welcom !</h1>
       ...
       <div x-data="{ show: false }"  x-init="setTimeout(() => show = true, 300)">
        <div x-cloak x-show="show" x-transition:enter="transition ease-out duration-700"
         ...
         <span class="text-white/60 font-serif italic">Dream Formation</span>
        </h1>
        </div>
    </div>
        ...
        <div class="mb-6" x-cloak x-show="show" x-transition:enter="transition ease-out duration-700"
         x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100">
      <p class="text-white/60 text-lg"> connect with top  trainers in your area</p>
        </div>
    </div>
        ...
        <div x-cloak x-show="show" x-transition:enter="transition ease-out duration-700"
         x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100">
      <a href="{{ route('register') }}" class="rounded-lg bg-white/10 px-4 py-2 text-white">create account</a>
