<x-app-layout>
    ...
    <div class="border-b border-white/10 pb-6">
     ...
     <form action="{{ route('job-vacancy.processApllication',$jobVacancy->id) }}"
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <x-input-label for="existing_{{ $resume->id }}" class="text-white cursor-pointer">
                    {{ $resume->filename }}
                    <span class="block text-sm text-gray-400">Uploaded on {{ $resume->created_at->format('M j, Y') }}</span>
                </x-input-label>
                </div>
                @empty
                    <p class="text-gray-400">You have no resumes uploaded. Please upload a new resume to apply.</p>
                @endforelse
