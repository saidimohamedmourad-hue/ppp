...
<form action="{{ route('job-vacancy.update', ['job_vacancy' => $jobVacancy->id, 'redirectTolist' => request('redirectTolist')]) }}" method="POST" >
                ...
                <div class="mb-4">
                    ...
                    <select name="companyId" id="companyId"
                        ...
                        @foreach($companies as $company)
                            <option {{ old('companyId',$jobVacancy->companyId) == $company->id ? 'selected' : '' }} value="{{ $company->id }}">
                                {{ $company->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="jobCategoryId" id="jobCategoryId"
                        ...
                        @foreach($categories as $category)
                            <option  {{ old('categoryId',$jobVacancy->categoryId) == $category->id ? 'selected' : '' }} value="{{ $category->id }}" >
        ...
        <a href="{{ route('job-vacancy.index') }}"
         class="text-gray-500 hover:text-gray-700">Cancel</a>
            ...
            <button type="submit" 
            class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">Edit JobVacancy</button>
        </div>
    </form>
