@php
    if (auth()->user()->role == 'admin') {
        $formAction = route('school.update', ['school' => $school->id, 'redirectTolist' => request('redirectTolist')]);
    } else if (auth()->user()->role == 'school-owner') {
        $formAction = route('my-school.update');
    }
@endphp
<x-app-layout>
    <x-slot name="header">
            ...
            <form action="{{ $formAction }}" method="POST">
                ...
                <div class="mb-4 p-6 bg-gray-100 border border-gray-300 rounded-lg shadow-sm">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="industry" id="industry"
                            ...
                            @foreach ($industries as $industry)
                                <option value="{{ $industry }}" {{ old('industry', $school->industry) == $industry ? 'selected' : '' }}>{{ $industry }}</option>
                            @endforeach
                        </select>
                        @error('industry')
                            <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="tel" name="phone" id="phone" value="{{ old('phone', $school->phone) }}" placeholder="+213 555 123 456" required
                        ...
                        <p class="text-gray-500 text-xs mt-1">Affiché aux candidats sur chaque formation.</p>
                        @error('phone')
                            <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" rows="4"
                            class="{{ $errors->has('description') ? 'outline-red-500 outline outline-1' : '' }} w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $school->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mb-4 p-6 bg-gray-100 border border-gray-300 rounded-lg shadow-sm">
                    ...
                    <span>Owner of the school</span>

                    <div class="mb-4">
                        <label for="owner_name" class="block text-gray-700 font-semibold mb-2">Owner Name:</label>
                        <input type="text" name="owner_name" id="owner_name" value="{{ old('owner_name', $school->owner->name ?? '') }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input disabled type="email" name="owner_email" id="owner_email" value="{{ old('owner_email', $school->owner->email ?? '') }}"
                    ...
                    <div class="mb-4">
                        <label for="owner_password" class="block text-gray-700 font-semibold mb-2">
                            Change Password: (leave empty to keep current)</label>
                ...
                <div class="flex justify-end space-x-4">
                    @if (auth()->user()->role == 'admin')
                        <a href="{{ route('school.index') }}" class="text-gray-500 hover:text-gray-700">Cancel</a>
                    @endif
                    @if (auth()->user()->role == 'school-owner')
                        <a href="{{ route('my-school.show') }}" class="text-gray-500 hover:text-gray-700">Cancel</a>
                    @endif

                    <button type="submit"
                        class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">Update School</button>
                </div>
            </form>
        </div>
    </div>
